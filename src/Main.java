...
public class Main {
	...
	public static void main(String[] args) throws RowsExceededException,
		...
		s1 = workbook1.createSheet("Resultados GRASP", 0);
		Label label = new Label(colunaAtual, linhaAtual, "Instancia");
		s1.addCell(label);
		colunaAtual++;
		label = new Label(colunaAtual, linhaAtual, "Custo Otimo");
		s1.addCell(label);
		colunaAtual++;
		label = new Label(colunaAtual, linhaAtual, "Custo GRASP");
		s1.addCell(label);
		colunaAtual++;
	...
	public static void ils(String nomeArq) throws RowsExceededException,
			WriteException {
		int CustoMedio = 0;
		...
		Label label = new Label(3, linhaAtual, CustoMedio + "");
		s1.addCell(label);
		colunaAtual = 0;
		linhaAtual++;
	}

	public static void grasp(String nomeArq) throws RowsExceededException,
			WriteException {
		int CustoMedio = 0;
		for (int seed = 1; seed < 10; seed++) {
			leArqInstancia(nomeArq);
			...
			corrente = buscaLocalRemoveFacilidade(corrente, random,
					custoConexao);
			Solucao best = corrente.copiarSolucao();
			int iter = 1;
			while (iter < 100) {
				corrente.ConstruirSolucao(custoConexao,
						custoAberturaFacilidade, random);
				corrente = buscaLocalRemoveFacilidade(corrente, random,
						custoConexao);
				if (best.getCustoTotal() > corrente.getCustoTotal()) {
					best = corrente.copiarSolucao();
				}
				iter++;
			}
			// System.err.println("Semente: " + seed);
		...
		Label label = new Label(colunaAtual, linhaAtual, nomeArq);
		s1.addCell(label);
		colunaAtual++;
